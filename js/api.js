...
const loadData = (url, onSuccess, onError) => {
  ...
  dataPromise.then((response) => {
    ...
    //тут костыли: Данные с сервера часто приходят без должных заголовков и они игнорируются, поэтому берем локалный JSON сверху.
    onSuccess(dataLocal);
    //И прячем сообщение об отсутствии данных
    document.querySelector('.load-error').style.opacity = '0';
  });
};
